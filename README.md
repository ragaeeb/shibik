...
![CodeRabbit Pull Request Reviews](https://img.shields.io/coderabbit/prs/github/ragaeeb/shibuk?utm_source=oss&utm_medium=github&utm_campaign=ragaeeb%2Fshibik&labelColor=171717&color=FF570A&link=https%3A%2F%2Fcoderabbit.ai&label=CodeRabbit+Reviews)
[![codecov](https://codecov.io/gh/ragaeeb/shibuk/graph/badge.svg?token=UVLXK99AT6)](https://codecov.io/gh/ragaeeb/shibuk)
...
```bash
bunx shibuk https://example.com/brand/ --out ./brand
```

Positional output path:

```bash
bunx shibuk https://example.com/brand/ ./brand
```
...
The build includes a post-step that rewrites `@/` path aliases in `dist/` to relative imports so `bunx shibuk` works from a published package without needing `tsconfig.json`.
