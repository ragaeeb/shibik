...
import { collectEmbeddedUrls, collectManifestAssets, collectNumericSequenceUrls } from '@/discovery.js';
...
export const main = async (argv = process.argv.slice(2)) => {
    ...
    await applyCapturedEntryHtmlForCandidates(originHost, outDir, entryHtml, capture.landingUrl, [
        config.url,
        capture.landingUrl,
        capture.finalUrl,
    ]);
    ...
    const allUrls = [...capture.urls, ...config.extraUrls, ...extraUrls].filter((value): value is string =>
        Boolean(value),
    );
    ...
    await writeRuntimeShim(outDir, originHost, runtimeCandidateUrls, entryPath);
