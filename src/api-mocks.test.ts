...
describe('isResponseMockCandidate', () => {
    it('should accept same-origin api paths', () => {
        expect(isResponseMockCandidate('https://example.com/api/session/start', 'example.com')).toBe(true);
    });

    it('should accept same-origin query endpoints without asset extensions', () => {
        expect(isResponseMockCandidate('https://example.com/v3/config/pages?url=/fr-fr/demo', 'example.com')).toBe(
            true,
        );
    });
