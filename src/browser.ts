...
import { type InteractionCandidate, selectInteractionTargets } from '@/browser-interactions.js';
...
const captureApiResponse = async (response: HTTPResponse, config: Config, captured: Set<string>) => {
...
const captureResponseArtifacts = async (response: HTTPResponse, config: Config, capturedApi: Set<string>) => {
...
const collectCtaCandidates = (texts: readonly string[], limit: number) => {
    ...
    for (const node of Array.from(document.querySelectorAll<HTMLElement>('button, a[href], [role="button"]'))) {
...
const clickInteractionTargets = async (page: Page, targets: CtaTarget[], landingUrl: string, timeoutMs: number) => {
...
const collectCaptureMetadata = async (page: Page, landingUrl: string, documentHtml: string) => {
...
const fetchDocumentHtml = async (urlStr: string, config: Config, cookieHeader: string) => {
...
const runCaptureNavigation = async (page: Page, config: Config) => {
    ...
    await sleep(config.idleWaitMs);
    return { documentHtml, landingUrl };
};
