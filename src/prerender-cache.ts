...
const PRERENDER_CACHE_REGEX =
  /<script\b[^>]*\bid=(['"])rb3-prerender-data-cache\1[^>]*>([\s\S]*?)<\/script>/i;
const INLINE_CONTENT_PANEL_MODULE =
  "rbgemc-rb3/inline-content-panel/inline-content-panel-controller";
...
export const extractPrerenderCacheEntries = (html: string) => {
  ...
  try {
    ...
    return Object.entries(parsed).filter(
      (entry): entry is PrerenderEntry =>
        typeof entry[0] === "string" && entry[0].startsWith("/"),
    );
  } catch {
    return [] as PrerenderEntry[];
  }
...
const collectInlineContentFallbackUrls = (
  value: unknown,
  origin: string,
...
export const persistPrerenderCacheMocks = async (
  html: string,
  origin: string,
  outDir: string,
  originHost: string,
