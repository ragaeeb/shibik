...
import {
  LEAF_TO_ROOT_FOLDERS,
  ROOT_TO_ENTRY_FOLDERS,
  TEXT_EXTENSIONS,
} from "@/constants.js";
import {
  directoryExists,
...
export const copyMissingTree = async (
  source: string,
  target: string,
  options: CopyMissingTreeOptions = {},
) => {
  ...
  for (const relativePath of nestedFilesGlob.scanSync({ absolute: false, cwd: source, dot: true })) {
