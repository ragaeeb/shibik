...
export const downloadUrl = async (
  ...
  try {
    ...
    log(
      "WARN",
      `Directory create failed: ${path.dirname(absPath)} (${getErrorMessage(error)})`,
    );
    return "failed";
  }
  ...
  for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
    try {
      ...
      if (attempt >= config.maxRetries) {
        log(
          "WARN",
          `Download failed (${attempt}/${config.maxRetries}): ${urlStr} (${getErrorMessage(error)})`,
        );
        return "failed";
      }
...
export const collectMissingDownloads = async (
  urls: string[],
  outDir: string,
  originHost: string,
) => {
