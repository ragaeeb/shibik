...
const SHIBIK_BOOTSTRAP_MARKER = '<!-- shibuk:bootstrap -->';
...
export const writePlaceholderForMissing = async (urlStr: string, outDir: string) => {
    ...
    if (ext === '.png') {
        ...
        body = EMPTY_SVG;
    } else if (ext === '.mp4' || ext === '.webm' || ext === '.mp3' || ext === '.ogg' || ext === '.wav') {
        body = new Uint8Array();
    } else if (isExternalAsset && ext === '.js') {
        body = '// Placeholder generated by shibuk for an unavailable external script.\n';
    } else if (isExternalAsset && ext === '.css') {
        body = '/* Placeholder generated by shibuk for unavailable external CSS. */\n';
