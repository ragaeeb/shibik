...
const buildRuntimeCode = ({ pathAliases, queryAliases }: RuntimeAliasMaps) => {
...
const writeRuntimeScript = async (outDir: string, aliasMaps: RuntimeAliasMaps) => {
  await writeTextFile(path.join(outDir, RUNTIME_FILE_NAME), buildRuntimeCode(aliasMaps));
};
...
export const writeRuntimeShim = async (
  ...
  await writeRuntimeScript(outDir, { pathAliases, queryAliases });
