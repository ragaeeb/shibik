...
const applyHostRewrites = (
  ...
  const protoLessResult = replaceIfMatched(
    nextContent,
    patterns.originProtoLess,
    context.contentRootSlash,
  );
...
const applyMarkupRewrites = (
  ...
  const result = replaceIfMatched(
    nextContent,
    patterns.baseHrefRegex,
    `$1$2${context.contentRootSlash}$2`,
  );
  nextContent = result.content;
  modified ||= result.modified;
...
const applyJavaScriptPatches = (content: string, filePath: string): RewriteResult => {
  ...
  const finalContent = nextContent.replace(
...
const createRewriteFileContext = (filePath: string, outDir: string): RewriteFileContext => {
  ...
  const externalParts = relativeToExternal.startsWith("..")
    ? []
    : relativeToExternal.split(path.sep).filter(Boolean);
...
export const buildAliasMap = (outDir: string): AliasPair[] => {
  ...
  return Array.from(aliasMap.entries()).filter(
    (entry): entry is AliasPair => typeof entry[1] === "string",
  );
};
...
export const rewritePaths = async (outDir: string, originHost: string, knownHosts: Set<string>) => {
  ...
  for (const file of walkDir(outDir)) {
    ...
    const result = applyRewritePipeline(
      content,
      aliasPairs,
      patterns,
      createRewriteFileContext(file, outDir),
    );
    if (!result.modified) {
      continue;
    }
